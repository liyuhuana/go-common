...
func (p *ApolloClient) start() (err error) {
	...
	if err != nil {
		log.Error("failed to get notification,err:", err)
		return
	}
	...
	for nameSpaceName, notifycationId := range notifyFiles {
		...
		if err1 != nil {
			log.Info("failed to get config form apollo,err:", err1, "namespace:", nameSpaceName)
			return err1
		}
		...
		if err1 != nil {
			log.Error("failed to rewrite config", configFile, "err:", err1)
			return err1
		}
...
func (p *ApolloClient) getNotification() (notifyFiles map[string]int, err error) {
	...
	if err != nil {
		log.Error("failed to marshal notifications,err:", err)
		return
	}
	...
	if err != nil {
		log.Error("从配置中心拉取配置修改信息失败，err:", err, "url:", uri, "code:", code)
		return
	}
	...
	for _, v := range result {
		...
		log.Warn("notification:", v.NamespaceName, v.NotificationID)
	}
	return
}
...
func (p *ApolloClient) GetConfig(nameSpace string) (content string, err error) {
	...
	if err != nil || code != http.StatusOK {
		log.Error("从配置中心拉取配置失败，err:", err, "url:", uri, "code:", code)
		return
	}

	err = json.Unmarshal(res, &result)
	if err != nil {
		log.Error("failed to unmarshal body,err:", err)
		return
	}
...
func (p *ApolloClient) AddConfig(configFiles []ConfigFile) (err error) {
	...
	for _, v := range configFiles {
		...
		if ok {
			log.Error("already listen config:", key)
			return
		}

		_, err = p.GetConfig(configFile.NameSpace)
		if err != nil {
			log.Error("failed to get config from apollo,err:", err)
			return
		}
...
func (p *ApolloClient) reWrite(configFile ConfigFile, data []byte) (err error) {
	...
	//空内容，不覆盖本地
	if len(data) == 0 {
		log.Warn("content is empty,config file:", configFile)
		return
	}

	//TODO 是否需要更安全的校验？
	fileName := strings.Join([]string{configFile.Path, configFile.NameSpace}, PathSeparator)
	log.Info("rewrite file:",fileName)
	err = ioutil.WriteFile(fileName, []byte(data), 0644)
	if err != nil {
		log.Error("failed to write file:", configFile.Path, "err:", err)
		return
	}
	return
}
...
func (p *ApolloClient) getConfigFile(nameSpaceName string) *ConfigFile {
	...
	if !ok {
		log.Warn("not exist namespace name:", nameSpaceName)
		return &ConfigFile{}
	}
